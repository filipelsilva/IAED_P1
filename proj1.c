 ...
 * Descricao: Sistema de logistica (Projeto 1)
 */
...
/* Altera o preço de um produto existente no sistema (comando 'p') */
...
/* Retorna a descrição e a quantidade de um produto numa encomenda (comando 'E') */
...
/* Escreve os produtos (ctrl: 1 -> armazem; 0 -> encomenda (ide) */
void print_produtos(int ctrl, int ide) {
    int i = 0;
    if (ctrl) {
        printf("Produtos\n");
        while(i < p_num) {
...
/* Lista todos os produtos existentes no sistema por ordem crescente de preço (comando 'l') */
...
/* Lista todos os produtos de uma encomenda por ordem alfabética da descrição (comando 'L') */
void lista_encomenda_nome(int ide) {
    if (teste_encomenda(ide)) {
        /* Sort dos produtos */
        mergesort(pedidos[ide].prods, 0, pedidos[ide].prod_num - 1, 0);
        /* Print pela nova ordem */
        print_produtos(0, ide);        
    } else {
        /* Caso a encomenda nao exista */
        printf("Impossivel listar encomenda %d. Encomenda inexistente.\n", ide);
    }
